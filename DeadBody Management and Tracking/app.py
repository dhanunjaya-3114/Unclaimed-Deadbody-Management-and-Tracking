...
def admin_by_default():
    check_admin = User.query.filter_by(u_email="admin@manwill.com").first()
    if not check_admin:
        ...
        db.session.add(
            User(
                u_email="admin@manwill.com",
                u_password=generate_password_hash("Admin@1025"),
...
def search_body():
    ...
    return render_template("search_body.html", results=results, selected_status=status)
...
def body_detail(body_id):
    ...
    return render_template("body_detail.html", record=record, claims=claims, role=current_role_name())
...
def edit_body(body_id):
    ...
    return redirect(url_for("body_detail", body_id=body_id))


@app.route("/claim/<int:body_id>", methods=["POST"])
@role_required("Public")
def submit_claim(body_id):
    body = DeadBody.query.get_or_404(body_id)
    if body.status == "Cremated":
        flash("Cannot submit claim for cremated record.", "error")
        return redirect(url_for("search_body"))
